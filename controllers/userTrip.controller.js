...
async function make_trip(req, res) {
    try {
        ...
        const exist_deliv = (await client.query(`select * from delivery where user_id = ${data.driver_id};`)).rowCount;
...
async function update_trip(req, res) {
    try {
        ...
        if (req.user.user_type == 'Normal_user') {
            ...
            const trip_count = (await client.query(`update user_trips set ${change_driver}${chang_distance}
                where item_id = ${data.item_id}`)).rowCount;
            res.status(200).json({ status: 'success', updated_rows: trip_count });
            return;
        }
