...
async function add_comp(req, res) {
    try {
        ...
        if (myItem === 0) {
            res.status(400).json({ mess: 'There is invalid date. Check on the id of losted item' });
            return;
        }
        const date = new Date();
...
async function showAll(req, res) {      //show all competitons without user's competitions
    try {
        const date = new Date();
...
export { add_comp, delete_comp, showAll };