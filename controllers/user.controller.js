...
async function get_available_car(req, res) {
    try {
        ...
        const car_data = (await client.query(`select brand,model,price,transmission from (cars join centers on cars.center_id = centers.center_id)
...
async function choose_car(req, res) {
    try {
        ...
        const can_rent = (await client.query(`call rent(${data.car_id},${req.user.user_id},${deliv_trip[0].balance});`)).rows[0].rent;
        if (!can_rent) {
